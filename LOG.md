...
**边界插曲（如实登记，事态仍在演进）**：作业期间根 `tests/`（21 项门禁）与 `docs/superpowers/`（8 份 spec/plan）
被从盘上删除（非本窗四代理所为，疑似并行窗）；因除删除外无其他改动，`git restore` 全量找回零丢失，
找回后根级 21 项复跑通过。**随后全仓盘点发现同波删除扩大到 gitignored 私有资料区**：
`CUMCM2026Problems/`（A 题调研/作战手册/规则与合规/盲审报告，唯一副本、不可再生）、`赛前试炼任务/`、
`方法武器库/` 三者从盘消失；时间窗锁定在本轮首次全量 pytest（717+1，目录尚存）之后约半小时内。
取证：回收站枚举 `total_recycle_items=0`（绕过回收站的脚本级删除）；桌面与用户目录 maxdepth 3~4
全盘搜目录名与特征文件（research_physics/DATA_FACTS/盲审）零命中。私有区无 git 副本，
可恢复性待并行窗核查（疑与"GMCM/modex 融合+整仓重构"在途窗口相关），本窗不擅自处置。
