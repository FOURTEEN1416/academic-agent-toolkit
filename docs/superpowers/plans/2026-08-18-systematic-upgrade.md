> ⚠️ **仅供追溯（2026-08-28 治理标注）**：本文为阶段性快照，其中数字基线/定位表述已被后续事实取代，以 `pytest.ini` 注释（公开侧口径真源）与最新实证为准（标注时点：pytest 225 passed、catalog 269 条/正式 10）；`dev-docs/truth-index.md` 为内部副本，不入库。本文件不再维护。
