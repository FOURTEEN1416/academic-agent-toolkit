# 数学建模全流程套件 — OpenCode 科研 Agent 系统

> 这是一套为 **OpenCode 桌面版**适配的科研/数模竞赛系统。
...
| "写基金申请书" | 私有扩展技能（不随公开包分发） |
| "写专利/软著" | 私有扩展技能（不随公开包分发） |
...
共享项目根 `D:\Desktop\数模竞赛\opencode.json` 使用 `数模专家` 作为默认 primary agent，显式扫描 `./数学建模全流程套件/skills`，并自动加载本文件作为项目指令。四个 agent 定义位于共享项目根 `.opencode/agents/`。`subagent_depth` 固定为 1：数模专家可以派发数模审稿人、数模视觉审查或数模编辑，但子智能体不能继续派发新的子智能体。`share` 固定为 `disabled`，竞赛题目、数据和产物不会被自动共享。
