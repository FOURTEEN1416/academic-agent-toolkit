...
def test_shared_project_opencode_configuration_selects_modeling_agent_and_skills():
    ...
    assert config["skills"]["paths"] == ["./数学建模全流程套件/skills"]
    assert config["instructions"] == ["数学建模全流程套件/AGENTS.md"]
...
@pytest.mark.skipif(AGENTS_MISSING, reason="宿主 agent 配置不随套件分发（由集成方提供）")
