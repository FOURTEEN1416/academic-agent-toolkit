...
- **受控 pyc 分发件**：`academic-toolkit/tools/` 下 14 个工具以 `.pyc` 分发（Python 3.11 字节码），**该 `.pyc` 即真源码**——同名 `.py` 是 `pyc_loader.py` 自动生成的薄包装器（缺 `.pyc` 即 exit 1），真源码从未随仓入库。故 `*.pyc` 必须 tracked（`.gitignore` 有 `!tools/*.pyc` 例外），严禁按"以 .py 为真源"删除。**字节码不可源码审查**：审计者读 `.pyc` 需借助支持 3.11 的反编译器，或运行 `.venv311`（本机 3.11 venv）原生执行验证行为。改动这 14 个工具须同时更新 `.pyc` 与包装器。
