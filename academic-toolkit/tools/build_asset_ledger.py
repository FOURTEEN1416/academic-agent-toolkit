...
def classify_path(relative_path: Path) -> tuple[str, str, str, str]:
    ...
    if normalized.startswith("academic-toolkit/engine/") or normalized.startswith(
        "academic-toolkit/tests/"
    ) or normalized.endswith("academic-toolkit/AGENTS.md"):
        return "candidate_public_core", "runtime_core", "project_authored_unverified", "blocked"
    if normalized.startswith("academic-toolkit/skills/"):
    ...
    if normalized.startswith("academic-toolkit/tools/"):
        return "experimental", "main_tools", "unverified", "blocked"
    if normalized.startswith("academic-toolkit/data/"):
        return "private_extension", "main_reference_data", "unverified", "excluded"
    if normalized.startswith("academic-toolkit/"):
