...
# 本文件是 academic-toolkit/tools/arxiv_fetch.pyc 的等源重建（原真码只在 pyc 里）。
...
_MIN_PDF_BYTES = 10240

_NEW_STYLE_ID_RE = re.compile('^\\d{4}\\.\\d{4,5}(v\\d+)?$')
_OLD_STYLE_ID_RE = re.compile('^[A-Za-z.-]+/\\d{7}(v\\d+)?$')
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    search_parser.add_argument('query', help='Search query or arXiv ID (bare ID or id:ARXIV_ID).')
    ...
    download_parser.add_argument('id', help='arXiv paper ID, e.g. 2301.07041 or cs/0601001')
    download_parser.add_argument('--dir', default='papers', metavar='DIR',
                                 help='Output directory (default: papers).')
    download_parser.add_argument('--delay', type=float, default=1,
