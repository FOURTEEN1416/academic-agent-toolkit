...
> （tests/test_asset_utilization.py 随 pytest 护航）。
> 非技能资产（数据/参考论文/工具脚本/参考图集）由 StepAction 的 `assets` 字段按步暴露
> （同 templates.json 维护），本文件只记技能。

## 使用规则

1. 每步开工：看 StepAction 的 `companion_skills`，按需加载 **1-3 个**（宁缺勿滥，防上下文噪音）。
2. **强制申报（C1 闸）**：complete 时 evidence 必须含 `companion_skills` 字段——
...
14 步主技能：`comp-problem-analysis` → `comp-literature` → `comp-modeling` → `comp-code` →
...
编排器与变体：`comp-pipeline`（8 阶段轻量编排）、`comp-paper-en`/`comp-paper-en-docx`/`comp-paper-zh-docx`（语言/格式变体）、`comp-compile-en`（英文编译）、`comp-statistics-topic`（统计建模赛专用模板）。

## 二、每步推荐（StepAction.companion_skills 同步给出）

> 步骤推荐槽位现为 12（S5 另含必用位 `paper-figure-palette`）——结构性冗余的 41 个技能全部移入 §三在册
> （修剪 ≠ 弃用，按需加载照常留痕），地图↔引擎集合一致性由
> `tests/test_asset_utilization.py::test_map_section2_matches_engine_companions` 机检锁定。
> 修剪依据全文见内部 LOG 续20/续21；利用率账本：`python tools/check_asset_utilization.py`。

| 步骤 | companion_skills | 一句话用途 |
|------|------------------|-----------|
| 1 赛题分析 | （无） | 五段产出与反 AI 陷阱登记已内嵌 comp-problem-analysis 契约 |
...
- 论文写作（5）：`anti-ai-detection`（词表级检测由 _utils/ai_tell_check 承担）、`comp-latex-writing`（cumcmthesis 模板内嵌）、`result-to-claim`（三要素转写为合同铁律）、`paper-plan-zh`（骨架由合同确定）、`format-profile`（cls 强制格式）
- 编译交付（4）：`docx-export`（output_format=docx 机制自动加步）、`docx-format-check`、`docx-template-map`、`latex-document`

**地图激活批（13 个）**——只差地图激活面的缺口技能：技能实体与 catalog 条目俱在、
只差路由面（无 StepAction 推荐位之外的第二路由通道）。已按情境可用接入本节，
并同步 catalog 条目 `disposition: "routed"` 处置字段。
...
- 文献/研究辅助（4）：`arxiv`（arXiv 检索/下载/摘要，工具线 `tools/arxiv_miner.py` 同域）、`lit-review-communications`（社区/非学术资料综述）、`comp-contest-research`（多轮深度调研管线，与模板 `deep_research` 同域但非其步骤主技能，故仅记 routed）、`lit-review-systematic`（学术综述方法模板）
...
**整技能收编（1 个）**——上游 Adkid-Zephyr/oral-paper-skill 收编
（无 LICENSE 文件如实登记，pinned a2c4bc4，status experimental；catalog disposition `routed`，
公开再发布待上游授权）：

- 学术写作（1）：`paper-oral-exemplar`（对照 883 篇顶会 Oral 蒸馏的七项做法做稿件对照改进/学习复盘，
  compare/reflect 双模式、≤3 条源链接建议、摘要级证据不越级；与 §二 `anti-defensive-writing`
  删-hedge 互补成对——先对照增补、后删冗余限定；含数模论文章节桥接表，修订轮/S13 终审前按需加载）

**无 Origin 渲染路线（1 个）**——fig-plot-edit（§四）方法论的
无 Origin 继承者：技能实体为本仓原创渲染代码，方法论/配色真源复用 `skills/fig-plot-edit`：

- 绘图/可视化（1）：`fig-plot-edit-lite`（没装 Origin 也要出版级数据图——逐列角色确认/方案哈希冻结/
  fig-plot-edit 官方配色目录/出版图形合同（白底 Arial 单栏 9cm/无标题/图例无框）移植到
...
| 科研选题与实验（10） | idea-discovery, idea-discovery-robotics, research-pipeline, research-refine, research-refine-pipeline, research-review, experiment-plan, experiment-bridge, monitor-experiment, run-experiment | 长周期科研管线，赛时 3 天用不上 |
...
| scholar 期刊投稿系（6） | arxiv-metadata, arxiv-submission-package, latex-bib-doi-toggle, lit-citation-verify-latex, doi-bibtex, openalex-search | arXiv/DOI 投稿工具 |
...
| Origin 可编辑绘图（1） | fig-plot-edit | 需本机 Origin/OriginPro 2021+ 商业软件 + Windows COM 自动化，非赛时工具；学术科研域按需（整技能收编自 hang-jin/editaplot，Apache-2.0；本机 Origin 未装、登记待用；runtime 引擎在 vendor/forks/editaplot 不入库） |
| 语料挖掘（1） | award-paper-mining | 获奖论文语料挖掘与统计沉淀（赛后/赛前语料建设用） |
...
- `pdf-toolkit`（PDF 合并/拆分/OCR/填表）——已随 `academic-toolkit/AGENTS.md` §三 路由表接入
...
## 六、统计（check_asset_utilization.py 机对账）
...
修剪只改变"哪一步主动推荐"（现 12 槽位），不改变任何技能的可知性——全部 41 个移入 §三在册。
**P4 技能绑定**：comp_cumcm 全部 14 步声明 `metadata.skill_binding`（`main_required` =
...
对账口径：技能 = skills/ 下含 SKILL.md 的目录（减 `_utils`/`shared-scripts`/`shared-references` 三个非技能目录）；
...
## 七、华为杯管线对照（comp_huawei）

> 引擎 `comp_huawei` 与国赛同构 14 步，
