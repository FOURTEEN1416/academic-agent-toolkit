...
"规则拦截"，导致全部工具调用被硬断（2026-09-09 会话锁死事故根因）。因此：
  ...
  2. 引导器层 fail-open（2026-09-10 定稿根治）："python 找不到/起不了脚本"发生
     ...
     try/except 兜底 fail-open，整类 exit-2 误判物理消失。定位顺序：
       a. env ZCODE_PROJECT_DIR/CLAUDE_PROJECT_DIR → <root>/academic-toolkit/hooks/本文件；
     ...
     双 cwd 故障史（a/b 各自对应的复现场景）：
       ...
       - 漂进仓库子目录（2026-09-10）：${ZCODE_PROJECT_DIR} 变量式写法在 cd 进
         academic-toolkit/ 后展开成双重拼接 academic-toolkit/academic-toolkit/hooks/... → 同样
...
def _audit_dir() -> Path:
    ...
    if root:
        ...
        # hooks/ 的上上级 = 仓库根（academic-toolkit/hooks/x.py → repo root）
