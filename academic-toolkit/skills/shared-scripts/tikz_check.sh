...
# 统一 Python 的 UTF-8 输出，避免 Git Bash + Windows 控制台把中文诊断打印成乱码。
export PYTHONUTF8=1
export PYTHONIOENCODING=utf-8

# 解析可用 Python：优先后端注入解释器，再尝试 python/python3/py -3。
# 使用 Bash 数组保存命令和参数，避免把 "py -3" 错当成一个可执行文件名。
PYTHON_CMD=()
if python -c "import sys" >/dev/null 2>&1; then
    ...
    echo "CRITICAL: 未找到可用 Python，TikZ 静态检查无法可靠执行"
    exit 1
fi
...
# 配色不绑定固定色板：黑白、单色、定性类别色和小面积高饱和强调都可合法。
# 静态检查只要求结构图通过语义令牌集中管理颜色；具体是否适合论文由最终 PDF
# 的对比度、灰度和视觉复核决定。
echo "INFO: 配色采用自适应策略，不强制色相、饱和度或模板色板"
...
#    文件路径经环境变量 TIKZ_CHECK_FILE 传入（不拼进代码，防路径含特殊字符/注入）。
legend_black=$(TIKZ_CHECK_FILE="$tikz_file" "${PYTHON_CMD[@]}" 2>/dev/null << 'PYEOF'
import re, os
tex = open(os.environ['TIKZ_CHECK_FILE'], encoding='utf-8', errors='ignore').read()
...
# 原始填充色过多通常意味着颜色在替代结构，但可能存在真实多类别语义，故只提示。
unique_fills=$(grep -oP 'fill=\{rgb,255:red,\d+;green,\d+;blue,\d+\}' "$tikz_file" 2>/dev/null | sort -u | wc -l)
if [ "$unique_fills" -gt 6 ]; then
    echo "WARNING: 发现 $unique_fills 种不同 rgb 填充色；确认每种颜色都有独立语义，且灰度/色盲环境仍可区分"
fi
# 命名色过多同样只提示，真正硬门由语义令牌检查负责。
named_fills=$(grep -oP 'fill=\w+!?\d*' "$tikz_file" 2>/dev/null | grep -v 'fill=none\|fill=white\|fill=gray\|fill=black' | sort -u | wc -l)
if [ "$named_fills" -gt 6 ]; then
    echo "WARNING: 发现 $named_fills 种不同命名填充色；避免无语义彩虹，必要时使用形状/线型冗余编码"
fi

# 结构图角色样式应引用论文级语义令牌；令牌本身不限制色相和色度。
PALETTE_CHECK=""
for _candidate in "$(dirname "$0")/tikz_palette_check.py" _utils/tikz_palette_check.py skills/shared-scripts/tikz_palette_check.py; do
    [ -f "$_candidate" ] && { PALETTE_CHECK="$_candidate"; break; }
done
if [ -n "$PALETTE_CHECK" ]; then
    if ! "${PYTHON_CMD[@]}" "$PALETTE_CHECK" "$tikz_file"; then
        critical=$((critical+1))
    fi
else
    echo "CRITICAL: tikz_palette_check.py 缺失，无法验证结构图颜色是否集中语义化管理"
    critical=$((critical+1))
fi

# 布局密度、极限缩放、节点过载和判断分支等结构级检查。
STRUCTURE_CHECK=""
for _candidate in "$(dirname "$0")/tikz_structure_check.py" _utils/tikz_structure_check.py skills/shared-scripts/tikz_structure_check.py; do
    [ -f "$_candidate" ] && { STRUCTURE_CHECK="$_candidate"; break; }
done
if [ -n "$STRUCTURE_CHECK" ]; then
    if ! "${PYTHON_CMD[@]}" "$STRUCTURE_CHECK" "$tikz_file"; then
        critical=$((critical+1))
    fi
else
    echo "CRITICAL: tikz_structure_check.py 缺失，无法验证布局密度与最终尺寸风险"
    critical=$((critical+1))
fi

# 明确会造成版面问题的禁止项检查。
if grep -q '\\fill\[.*rgb.*rounded corners' "$tikz_file" 2>/dev/null; then
    echo "WARNING: 发现手工大背景 \\fill；若它是组框，优先考虑 fit + background layer 自适应真实节点边界"
fi
...
# opacity 风险检测：源码级正则无法可靠区分装饰线、半透明实体和文字，故只提示视觉复核。
...
if [ "$faded_any" -gt 2 ]; then
    echo "WARNING: $faded_any 处裸 opacity ≤ 0.5；确认它们只用于非文字的辅助构件，并在最终尺寸下仍可辨"
fi

# 原始命名色过多提示：真实多类别编码可以超过三色，不设固定三色硬上限。
...
if [ "$draw_colors" -gt 5 ]; then
    echo "WARNING: \draw 命令直接用了 $draw_colors 种命名色；确认每种颜色均编码独立类别，并有形状/线型冗余"
fi
...
if [ "$red_total" -gt 2 ]; then
    echo "WARNING: $red_total 处文字直接使用纯红色；确认红色具有领域语义且不是装饰，并检查灰度可读性"
fi
...
if [ "$tiny_offset" -gt 3 ]; then
    ...
    echo "  → 单标签建议从 6pt/0.22cm 起步，多标签场景使用引线或 8pt+ 并在 PDF 中复核"
fi
...
# 白色文字可用于有充分对比的深色小面积强调节点，不直接判错。
grep -q 'text=white' "$tikz_file" 2>/dev/null && echo "INFO: 发现白色文字；请确认其背景对比度和灰度打印可读性"

# 旋转文字
grep -q 'rotate=90' "$tikz_file" 2>/dev/null && echo "WARNING: 发现 rotate=90，建议水平文字"

# 箭头粗细、圆角与否由图的语义和论文风格决定，不作模板化要求。
