...
- ⛔ **判据方向：只要闭合 `$$` 那一行后面还有非空白字符，就是问题**——不要只查全角括号、也不要只认纯数字编号：`(8′)`、`(A-1)`、`(6) 补充说明` 同样是尾巴（纯数字编号引擎还能兼容，非纯数字才吞后文）。
- ⛔ **不要只把全角 `（n）` 换成半角 ` (n)`**——那只是从"一定出事"换成"暂时不出事"，编号一旦带撇号（如 `(8′)`）立刻复发。编号一律移进公式内部改成 `\tag{n}`，让开启与闭合的 `$$` 都独占一行、前后不带任何字符。

三条扫描 grep（第①条会把 `$$x=y$$` 这类正常开块行报出来，属正常噪声，逐条看"`$$` 后跟的是编号/文字而不是 LaTeX"才是真问题）：
...
**Check D2 升级（块公式编号扫描的三分类）**：扫描时把块公式分**三类**报出——✅ 已用 `\tag{}`、⛔ 块外编号（要改成 `\tag{}`）、⛔ 完全未编号（要补）。⛔ **块外编号不算"已编号"**（这类写法会一路溜到导出）。另注意三条实测教训：

1. **文件名走 argv**（`python3 - "$TARGET_FILE" <<'PY'`）：heredoc 是单引号 `<<'PY'`，shell 不展开 `$TARGET_FILE`，写在脚本里会去打开字面名文件。
