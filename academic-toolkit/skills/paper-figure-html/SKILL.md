...
工作流类型决定数量：
- **数模竞赛 / 科研流程**（有 PROBLEM_ANALYSIS.md）：按清单全部生成（roadmap + flow_q1/q2/pipeline 等），**至少 1 张 fig_roadmap 技术路线图**。⛔ 凡画 `fig_roadmap*`/`fig_technical_route*`，动手前必须完整读取 `references/roadmap_quality.md`。
...
echo "🎨 风格种子 SEED=$SEED  STYLE_FAMILY=$STYLE_FAMILY（$_FAM_NAME）  H0=$H0°  TONE=$TONE  RADIUS=$RADIUS  ARROW=$ARROW  NODEACC=$NODEACC  SECT=$SECT  LAYOUT=$LAYOUT（全篇共用）"
```
...
**几何自检测三类问题（都在 `.fig` 内、渲染公式之后测，所以准）：**
| 类型 | 含义 | 常见成因 |
|---|---|---|
...
### Step 4.6: 科研规范自检（⛔ 每张必跑，FAIL 必修）

Step 4.5 的几何自检只量**图内部**的相对关系，**量不到"这张图缩进论文之后字还看不看得清"**。这是它的结构性盲区，也是"图糊成一坨、没重点"的第一性原因（上游真实工作区实测：6 张几何自检全 PASS 的图，5 张栽在这一步，最糟一张字只剩 4.5pt）。所以本步必须单独跑。

本仓当前 `tools/screenshot_capture.py` 未提供 `--norm-check` 机器闸，故按下列判据**人工量**（含公式的图先 `--render-math` 出 PDF 再量）；日后工具引入 norm-check 时改为机器执行：

| 项 | 判据 | 硬拦？ | 规范 |
|---|---|---|---|
| **字号** | `设计宽 ÷ 最小字号 ≤ 56`（缩后 ≥7pt） | ✅ 硬拦 | 见《两条会互相打架》节 ① |
| **占页** | `高/宽 ≤ 1.05`（占正文区高 ≤60%） | ✅ 硬拦 | 同节 ② |
| **普通图模块数** | ≤ 22 个 | ⚠ 只警告 | 同节 ③ |
| **文字对比度** | 普通文字对背景 ≥4.5:1（本仓更严：主文字 ≥7:1） | ✅ 硬拦 | WCAG 2.2 / 本仓 1058 行 |
| **路线图认知密度** | 语义节点 ≤18、文字模块 ≤26、全图字符 ≤560、单节点 ≤64；同时必须看得出共享内核和每问关键变换 | ✅ 硬拦 | **`references/roadmap_quality.md`（路线图必全文读）** |
| **路线图视觉重量** | 焦点 ≤1、无黑底节点、深色面积 ≤8%、粗边节点 ≤20% | ✅ 硬拦 | 同上 |
| **无斜体** | 无 `font-style:italic`／`<i>`／`<em>` | ✅ 硬拦 | 设计规范 8.1 |
| **可读性** | 文字盒无底纹（`background-image`）、无描边光晕 | ✅ 硬拦 | 底纹铁律节 |
| **内容** | 无截断／元内容／工程语汇／残留符号／公式留空 | ✅ 硬拦 | 内容红线节 |

⛔ 允许用户所选风格族使用合规中性色，但渲染后的普通文字必须达到对比度门槛。黑白族仍严格零彩色；也不得用大面积黑底、反色节点和粗边制造层级。

**⛔ 修的时候按这个优先级（顺序反了会来回打架）：**

1. **字号不过** → ①删模块（首选，同时降密度）②把长公式/参数从主标题移到副标题 ③提字号 ④缩窄节点 `min-width`。⛔ **别把图改成竖排**——那会撞穿占页上限。
2. **占页不过** → **压行数**：把编号标签并进名称行、副标题合并、缩 `padding`/`gap`。⛔ **别改朝向**，竖排只会更高。也可**适度加宽**节点（宽度有余量时，加宽能直接压低比例）——但要回头复查第 1 项。
3. **两项同时不过** → 说明内容量超出单图容量，**拆成两张图**或删到 5~8 个模块。
4. **内容不过** → 按内容红线节逐条重写「哪条文字 + 属哪类 + 为什么」。⛔ **别用缩短文字的办法糊过去**——截断本身就是被拦的那一项。
5. **可读性不过**（底纹压字/描边光晕）→ 把 `background-image` 从**有文字的盒子**上整条删掉，层次改用线宽（1.3/2.2/2.4double）+ 虚线 + 字号差 + 留白；同时删掉配套的 `-webkit-text-stroke` 白描边。⛔ 别改成浅一点的底纹——只要文字盒有 `background-image` 就拦。

改完回 Step 3 重出 PDF → 重跑 Step 4 / 4.5 / 4.6，直到三项都过。**FAST_MODE=1 时本步照跑**（纯测量、几十毫秒、不花 API），只跳 vision。

> 配色纪律以本仓风格种子 + 对比度门槛为准；上游 `data-mh-palette`/`data-mh-quality` 标记契约绑定上游分发工具，本仓模板不含、不采用。
...
**⛔⛔ 尺寸不再手填：追加完所有 include 块后，必须跑 `fig_include_size.py`**。数据图按真实长宽比分档；流程图/架构图/路线图还会读取 PDF 内有效字号、文字量和文本块数，自动决定简单图是否需要铺满、密图是否需要放大。`--strict` 会阻止"通栏后仍看不清"或竖长逻辑图继续进入论文：
...
**⛔⛔ 尺寸铁律**：不要手改脚本算出的宽度。简单图可能只需 `0.80~0.86`，一般图约 `0.88~0.94`，密图才接近 `0.96~0.98`；竖长或通栏后仍不足 7pt 时必须回源重排。`height=0.8\textheight` 只是防溢出上限。更根本的解法在**图本身的布局**：
...
⛔⛔ **几何自检 `--geom-check` 只抓"重叠/溢出/越界"，抓不出"参差不齐"**（列宽不一、行列没对上、边缘不成线，geom-check 全 PASS 但一眼就丑）。所以对齐必须**在写 HTML 时用下面的骨架从结构上保证**，不能靠出图后补救。
...
## modex-3 增补节（2026-09-10 同源对照吸收，出图前硬约束）

> 来源：Modex v3 paper-figure-html（溯源见 skills/shared-scripts/UPSTREAM.md）。以下七节为本仓原版未覆盖的出图质量硬约束（物理可读性/零灰墨色/文字选材/底纹描边/流程图标准/CSS 实测坑/配色配方），与本仓上方 Output Contract 并行生效。

### 0.1 物理可读性（⛔ 硬约束，三族同守；`--norm-check` 会硬拦）

**这是"图糊成一坨、没重点"的第一性原因，比布局审美更底层。** 上面 0 节和后面 B~G 节管的都是**图内部**的相对关系（层次、配色、对齐）；本节管**图缩进论文之后**的绝对尺寸——`--geom-check` 量不到这一维（它只看图内部），所以必须单独设闸。

**实测依据**（真实工作区 6 张已生成图，最小字号全是 11px）：**5 张不达标**。最糟一张设计宽 968px，插进 Word 缩到 0.48 倍，`11px` 的字只剩 **4.5pt**——低于印刷可读下限，眼睛分辨不出字形，只看到一片灰块。这就是"一坨"的字面成因。

**两条硬约束（缺一不可，会互相打架）：**

**① 可读性**：`设计宽度(px) ÷ 最小字号(px) ≤ 56`

推导：论文插图显示宽固定 5.5in（= `md_to_docx.py` 的 `_DEFAULT_IMAGE_MAX_W_IN`；LaTeX 走 `0.85\textwidth` 算出来是 55，两条路径几乎一致，所以这条通用）。于是 `字磅值 = 396 × 最小字号px ÷ 设计宽px`，要 ≥ **7pt**。查表即可：

| 最小字号 | 设计宽上限 | 适用 |
|---|---|---|
| 11px | **616px** | modern 族副标题（绝对下限，不许更小） |
| 12px | **672px** | 通用安全值 |
| 13px | **728px** | modern 族节点正文 |
| 14px | **784px** | mono / plain 族节点正文（禁副标题，字大词少） |

**② 占页**：`高/宽 ≤ 1.05`（即占正文区高 ≤ 60%）。超了会挤走正文、figure 浮动错位、前页留大片空白。

⛔⛔ **两条会互相打架，只治一条必翻车（实跑坐实）**：把 968px 宽图改成竖排，宽度问题解决了（字 14.8pt），但比例从 0.25 拉到 **1.45 → 占页 82%**，几乎占满一整页。**正解是压行数**（如把"M1 · 问题 1"这种编号标签并进名称行，三行制→两行制），改完 348×323、比例 0.93、字 13.7pt，两项同时过。**绝不是换朝向**——换竖排只会更高。

**③ 模块数与认知密度**：普通结构图建议 ≤22 个；**技术路线图硬上限 18 个语义节点**，并同时限制全图可见文字 ≤560、单节点 ≤64 个字符。紧凑总览通常 180–320 字，解释型总览通常 260–480 字；不能为了过密度检查删成只有问题标题的空提纲。字号合格只说明“能辨认”，信息充分才说明“能读懂”；超预算时拆成路线总览+真实求解架构图。

**⛔ 常见的三个撑宽元凶**（都真实翻车过）：
1. **阶段/模块横向铺开**：4 模块 × 158px + 3 箭头 × 92px = 968px。骨架变化必须留在**竖排范式内部**（见《骨架池》）。
2. **图内写说明段落**：`.note{max-width:900px}` 这种——它本身就在撑宽度，而且那段话属于 `\caption`，不该进图。⛔ **图内禁出现整段说明文字**。
3. **副标题字号压太小**：`.sub` **不得低于 11px，且不低于正文 2px 以内**（见 D.1 ③）——字号过小配宽图必糊。
### 0.2 零灰墨色（⛔ 硬约束：禁用中间灰。这是"一眼看出同一套系统"的最大破绽）

**⛔⛔ 铁律：`.fig` 内禁止出现任何「中间灰」。** 判据（含近灰，别只看 `r==g==b`）：

> 某颜色的 `chroma = max(R,G,B) − min(R,G,B) ≤ 18` **且** 明度 `lum = (299R+587G+114B)/1000` 落在 `(12, 243)` 之间 → **判为中间灰，禁用**。

只允许：**纯黑**（lum ≤ 12）、**纯白**（lum ≥ 243）、以及 B 节 `H0` 派生的**有明确色度**的强调色（chroma > 18，且用量 ≤15%）。

**⛔ 实测依据（这条不是审美偏好，是被用户当场认出来的签名）**：12 张结构、形态、排版轴全都不同的图，仍被一眼指出"老是弄灰色，很明显"。审计发现**它们共用同一套灰**——`#141414 / #8f8f8f / #c4c4c4 / #fafafa / #9a9a9a / #2b2b2b`，**11 种灰、106 次出现、每张 5~8 种**。根因不是"喜欢灰"，而是**灰成了唯一的层次工具**：底色灰、边框灰、副标题灰、箭头灰，五档"层次手法"最后全落到"深灰 vs 浅灰"的明度差。

**⛔ 那么"弱"怎么表达？** 用下面这些**非颜色**手段（`INK` 轴六档，由种子选一档，全篇统一）：
...
**⛔⛔ 底纹的写法与唯一合法位置：铺在【没有文字】的窄条/色块上。**

⛔ **底纹只铺在【没有文字】的窄条/色块上**——含文字的盒子加底纹，45° 斜线会垫在 11px 中文后面、笔画被网格切碎，再加白描边补救 → 一圈光晕。**这是"图丑"的第一位真因**，详见 0.4 节。`--norm-check` 的「可读性」项会硬拦。

```css
/* ✅ 合法：底纹铺在无文字的窄条（泳道带、分区色条、图例小方块） */
.rail{ width:7px;
  background-image:repeating-linear-gradient(45deg,#000 0 0.6px,transparent 0.6px 5px); }
.swatch{ width:14px;height:10px;
  background-image:radial-gradient(#000 0.5px,transparent 0.5px); background-size:4px 4px; }

/* ❌ 违规：任何【含文字】的盒子都不许有 background-image */
.n     { background-image:repeating-linear-gradient(45deg,#000 0 .6px,transparent .6px 5px) }  /* ❌ */
.n.sup { background-image:radial-gradient(#000 .5px,transparent .5px) }                        /* ❌ */
.n>*   { -webkit-text-stroke:2.6px #fff }   /* ❌ 上面两行的补救手段，一并禁 */
```

**⛔ 那么"填充感"怎么办**：不要填充感。文字盒一律纯白底，三级重量改用**边框**（`0.9px dashed` / `1.3px solid` / `2.2px solid` / `2.4px double`）+ **字号差 2px** + **`letter-spacing:.04em`** + **`padding` 递增**。实测这四样叠加足够拉开层次，且印刷更干净。

**⛔⛔ 副标题/次要文字的"弱"：只用「字号差 + 字距 + 引导符」，⛔ 不许调灰、⛔ 也不许用 `opacity`。**

⛔ **次要文字禁用 `opacity`**：黑字 `opacity:.78` 在白底上**合成明度 56**——落在禁灰区间、**肉眼就是深灰**，只是字面量写着 `#000` 骗过了源码扫描。`--norm-check` 的「零灰（合成）」项会硬拦它。分界线约在 `.82`，更淡即被拦。

```css
/* ✅ 三个手段叠加，全程纯黑 */
.n  .ttl { color:var(--tx); font-size:13px; }
.n  .sub { color:var(--tx-sub); font-size:11px;      /* ① 字号差 2px（0.1 节的上限） */
           letter-spacing:.04em; }                    /* ② 字距松开 = 视觉更轻 */
/* ③ 引导符：<span class="sub">— 参数标定</span>，破折号/括号本身就在说"这是附注" */

/* ❌ .n .sub{ color:#8b8b85 }        近灰：chroma=6、lum=138 → 违反本节 */
/* ❌ .n .sub{ color:#000;opacity:.78 } 合成 lum=56 → 违反本节 */
```

⛔ **字重（`font-weight`）慎用**：加粗是"一眼看出同一套系统"的特征之一，只在**单张图内最多一处**焦点上用，别当常规层次手段。

⛔ **需要"极轻"的大字装饰**（如背景层大编号）：用**空心字**，⛔ 不用 `opacity` 也不用浅灰。
```css
.bignum{ font-size:34px; color:transparent;
         -webkit-text-stroke:0.8px #000; }   /* ✅ 纯黑描边、内部不填充 */
/* ❌ .bignum{ color:#000; opacity:.18 } —— 合成 lum=209，落禁灰区间 */
```

**⛔⛔ 颜色必须走 CSS 变量，源码里不写死任何十六进制**：

```css
:root{ --bg-sup:#fff; --bd-sup:0.9px dashed #000; --tx-sup:#000;
       --bg-key:#fff; --bd-key:1.6px solid #000; --tx-key:#000;
       --bg-foc:#fff; --bd-foc:1.6px solid #000; --tx-foc:#000;
       --rule:#000;   --tx-tag:#000; --tx-mk:#000; --tx-sub:#000; }
```
理由（真踩过）：若把墨色规则**追加在样式表后面靠层叠覆盖**，被覆盖的旧灰声明**仍留在源码里**，自查时会误判"灰还在"甚至"灰变多了"（实测 106→114）。变量制让**全文件只有 `:root` 一处定义颜色**，改一处即全改。⛔ `:root` 必须放样式表**最前面**（变量先定义后引用）。

**⛔ 用户手选了彩色方案时**：仍守本节 —— **纯黑白骨架 + 单一强调色**，强调色只染**焦点边框与连线**（`hsl(H0,72%,38%)` 一类，chroma 足够大），其余全纯黑白。⛔ **不许把"强调色"稀释成浅灰调**。

**⛔ 自查（每张必做，比对照清单更硬）**：把 HTML 里所有 `#rrggbb`/`#rgb` 取出来逐个按上面公式算 chroma 与 lum，**任何一个落进"中间灰"区间即为违规**（如 `#777`：chroma=0、lum=119 → 违规），改用本节手段替代。⛔ **三位写法 `#777`/`#fff` 也要展开成六位再算**（只匹配六位会漏，真漏过）。⛔ **还要算 `opacity` 的合成值**：白底上 `color:#000;opacity:a` 的合成明度 = `255×(1−a)`，`a<0.82` 即落禁灰区间。
### 0.3 文字选材（⛔ 硬约束：图里的每句话都要能被论文读者独立读懂）

**病根**：图上的文字是从工作区中间产物（`MODELING_REPORT.md` 等）里抽出来的，**抽的时候把"给自己看的话"一起搬进了正式插图**。⛔ 实测 12 张真实图：**截断 39 处、元内容 18 处、公式剥离留空 4 处**。论文读者看不到中间产物，这些文字对他们就是天书。

**六类禁止内容**（`--norm-check` 的「内容」项逐条硬拦，会印出「哪条文字 + 属哪类 + 为什么」）：

| 类 | 长什么样 | 为什么不行 | 改法 |
|---|---|---|---|
| **元内容** | 「修正项 F1」「硬约束清单 HC-01..HC-08」「符号表」「参数口径表」「METHOD_CLAIMS」 | 讲的是**报告自身的修订记录/编号台账**，不是模型方法。「F1」「HC-01」只在中间产物里有定义 | ⛔ **整条换掉**，写它背后的**模型方法本身**（如「功率平衡约束」而非「修正项 F1」） |
| **截断** | 「问题 3 模型：满足 的最…」「姿态 → 阻力面积的映射与其…」 | 正式论文里读不通，读者卡在半句话上 | ⛔ **别硬切加省略号**。换**短标题**（自己重写一个 ≤14 字的），或**语义改写**保住完整意思 |
| **公式留空** | 「两种受限机制与 的非单调性」「体积分数　时的导通概率」 | 剥 `$...$` 时把符号删了没补，句子缺主语 | **保留符号**：要么留 LaTeX 走 KaTeX 渲染，要么写成纯文本符号（`v_p` → 「速度 v_p」） |
| **路径/标识符** | 「comp-paper-zh」「md_to_docx.py」 | 目录名/变量名不是论文内容（实测真被当成「本文重点」画进图） | 换成**中文术语**。⛔ 全大写公认缩写（MILP／NSGA-II／GCRS）不在此列，可以用 |
| **Markdown 残留** | 「结果约束清单\`）」「\*\*关键步骤\*\*」「- 参数标定」 | 抄文字时把源码符号一起搬来了 | 清掉反引号／星号／列表符／井号，只留正文 |
| **工程语汇** | 「口径」「闭环」「链路」「落地」「抓手」「颗粒度」 | 工程口语，学术图不用；**这是"一看就是 AI 写的"的主要来源** | 查《写作规范·工程/质检语汇泄漏》替代表（口径→定义／取法、闭环→反馈校正、链路→传递路径） |

**⛔ 抽文字时的正确姿势**（预防比返工便宜）：

1. **先读懂再写**，别复制粘贴。看完 `MODELING_REPORT.md` 的某节，用**自己的话**写一个 ≤14 字的标题。
2. **判断标准是"给答辩评委看"**：这句话他没读过你的中间文件，能不能懂？不能就重写。
3. **长度不够就删内容，不是切文字**。一个节点装不下就说明它该拆成两个，或者这个细节根本不该进图（移到 caption 或正文）。
4. **符号跟着公式走**：决定不渲染 LaTeX 时，把符号改写成纯文本（`$\phi$` → `φ` 或「体积分数」），⛔ 别只把 `$...$` 删掉。
...
⛔ **实测翻车过程**（必须记住这条因果链）：0.2 节禁灰之后，"表示重点"看似只剩黑底纹一条路 → 把 45° 斜线垫在 11px 中文后面 → **笔画被网格切碎、糊成一团** → 为救可读性再加 `-webkit-text-stroke:2.6px #fff` 白描边 → **每个字一圈白光晕，一眼就是机器生成**。用户原话：「为啥会出现那么丑的图啊」。

```css
/* ❌ 绝对禁止：底纹垫在文字后面 */
.n.key{ background-image:repeating-linear-gradient(45deg,#000 0 .6px,transparent .6px 5px) }
.n.key>*{ paint-order:stroke; -webkit-text-stroke:2.6px #fff }   /* ❌ 这是上一行的补救，一并禁 */

/* ✅ 文字层次只用这四样（全部零灰、无底纹） */
.n.sup { border:0.9px dashed #000 }      /* 弱：虚线边 */
.n     { border:1.3px solid  #000 }      /* 常规：细实线 */
.n.key { border:2.2px solid  #000 }      /* 重：粗实线 */
.n.base{ border:2.4px double #000 }      /* 基座/全局支撑：双线 */
/* 再叠：字号差 2px + letter-spacing .04em + padding 递增（留白本身就是层次） */

/* ✅ 底纹的正当用法：铺在没有文字的窄条上 */
.rail{ width:7px; background-image:repeating-linear-gradient(45deg,#000 0 .6px,transparent .6px 5px) }
```

**唯一例外 —— 空心字**：`color:transparent` + `-webkit-text-stroke` 没有填充、不产生光晕，放行（用于背景层大编号）。`--norm-check` 的「可读性」项按此豁免。

`--norm-check` 会**渲染后**逐个文字元素读 `backgroundImage` 与 `webkitTextStrokeWidth`，命中即硬拦并印出是哪条文字。
### 0.5 流程图另有一套标准（⛔ 别拿路线图的多层格式套流程图）

路线图与流程图是**两种图，两套规范**，混用是常见错误：

| | 技术路线图 | 求解流程图 |
|---|---|---|
| 依据 | Phaal / EIRMA **多层格式** | **ISO 5807** + 可读性通则 |
| 本质 | 多个层沿推进轴对齐，层间连线表示"下层支撑上层" | 单一控制流，按执行顺序推进 |
| 形状 | 层带/泳道/矩阵，形状不承载语义 | **形状即语义**（见下表），不许乱用 |
| 判断 | 无 | 菱形，**每条出边必须标是/否** |
| 起止 | 无需显式 | **必须有明确起点与终点**（胶囊形） |

**⛔ ISO 5807 形状语义（流程图专用，不许自创）**：

| 形状 | 含义 | CSS 实现 |
|---|---|---|
| 胶囊（圆端矩形） | 起点 / 终点 | `border-radius:999px` |
| 矩形 | 处理步骤 | 直角矩形 |
| 菱形 | 判断 | `transform:rotate(45deg)` 的方块 + 反向旋转的内层文字；⛔ 或用「矩形 + 左右尖角伪元素」避开旋转（旋转会让中文字面歪，且与禁斜体的观感冲突） |
| 平行四边形 | 输入 / 输出数据 | `clip-path` |
| 波浪底矩形 | 文档 / 报表 | — |
| 圆圈 | 断点连接符（图内跳转） | `border-radius:50%` + 内写编号 |

**⛔ 流程图硬规则**（`--norm-check` 的几何/内容项能兜住一部分，其余靠自查）：
1. **主流向单一且一致**：整图统一「上→下」或「左→右」，⛔ 不许一半向下一半向右。
2. **箭头单向、方向即语义**：箭头**尖端指向后继**。⛔ 不许出现无箭头的裸线（读者无法判断方向）。
3. **判断出边必须标注**：菱形每条出边写「是 / 否」（英文图 `Y / N`）。⛔ 只画两条线不标注 = 读者猜。
4. **⛔ 不许出现交叉线**：交叉即布局失败，用 flex/grid 重排或改走断点连接符（圆圈 + 同编号）。
5. **迭代必须画回边**：有收敛判断就必须有一条**回到上游**的箭头。⛔ "循环"只在正文写、图上不画，是最常见的失真。
6. **框内只写短动作短语**（≤10 字，如「校验输入」「更新权重」），⛔ 不写整句；长说明移交 `\caption`。
7. **必须有起点与终点**，且每条分支都要走到终点，⛔ 不许有断头路。
8. **⛔ 不靠颜色传达含义**：本 SKILL 默认纯黑白，恰好满足无障碍要求；分支靠**文字标签**区分，不靠色。
9. **异常/拒绝路径要画**：只画理想通路是常见遗漏（如"不收敛则回退"这一支）。

**⛔ 菱形的中文陷阱**：`rotate(45deg)` 的菱形里若不给内层文字反向旋转，中文会歪成斜体观感，与 0 节 8.1 条禁斜体冲突。**优先用"矩形 + 左右尖角"** 代替真菱形 —— 语义一样、中文不歪、CSS 更稳。

---

**⛔⛔ 流程图的拓扑轴 `FCTOPO`（与路线图的 `TOPO` 平行，⛔ 两者不通用）**

⛔ **为什么必须单列一条**：路线图的 6 种拓扑（层带/泳道/矩阵…）**没有一种适合流程图** —— 流程图是单一控制流，讲的是执行顺序与分支回环，不是"层与层的支撑关系"。若不给流程图自己的骨架轴，AI 会把每张都画成"竖着排的一串框"。

```bash
FCTOPO=$(( (SEED / 131) % 4 ))   # 流程图骨架
FCDIR=$((  (SEED / 101) % 2 ))   # 主流向: 0上→下 1左→右
FCFB=$((   (SEED / 107) % 3 ))   # 回边落点: 0回上一步 1回起点 2回中途步
echo "🔀 流程图 FCTOPO=$FCTOPO FCDIR=$FCDIR FCFB=$FCFB"
```

| `FCTOPO` | 骨架 | 适用内容 | 实测尺寸 / 占页 |
|---|---|---|---|
| 0 | **竖排 + 右侧独立回边轨道** | 顺序算法带 1~2 处迭代收敛 | 548×573 · 59% |
| 1 | **蛇形折行 + 旁路 fork-join** | 步骤多（≥5）且有交叉验证支路 | 582×261 · 25% |
| 2 | **嵌套层分列**（每层一列，列内闭环） | **多层嵌套迭代**（内层积分/中层步长/外层寻优） | 591×366 · 35% |
| 3 | **竖排 + 中段双列真并行** | 主干顺序但中段有**双链并行**再汇合比对 | 568×591 · 59% |

**⛔ FCTOPO 的硬性依赖**：
1. `FCTOPO=1`（蛇形）**必需**：横排节点数 ≥5。⛔ 实测左→右横排 7 个节点需 950px，远超 616 上限，不折行必翻车。
2. `FCTOPO=2`（嵌套分列）**必需**：内容真有 ≥2 层嵌套迭代。**这个骨架的额外好处是"多条回边天然不交叉"** —— 每层回边只在本列内闭合，不必排轨道。
3. `FCTOPO=3`（双列并行）**必需**：真有两条并行链（如"解析播种 + 全局搜索"互为交叉验证）。⛔ 只有一条链时回落 0。
4. `FCTOPO=0` 是**唯一无前置条件**的骨架 → 所有回落都落到它。
5. ⛔ **回边条数由内容定，不由种子定**：算法无迭代就**不画回边**（实测一份报告明确"搜索半径保证零漏检"，该图回边数为 0，种子给的"回边回起点"作废）。

**⛔ 判断节点数同样由内容定 —— 且要先分清"判断"与"计算"**：
> 实测踩过：报告里 `Y = 𝟙[...]` 是**指示函数求值**（算一个布尔量），我为了凑"种子要求 1 个判断"把它画成菱形 + 两条出边 + 两个结论盒，属内容失真；连带还漏画了汇合线。
> ✅ **判据**：只有"**后续走哪条路取决于此结果**"才是判断（菱形）；算出一个值供下游使用的，一律是处理（矩形）。
### 0.6 CSS 实测坑清单（⛔ 每条都真翻车过，照抄避坑）

**① 多列 grid 必须显式 `grid-column`，不能靠自动流式排布**
每行只供「标签 + 内容」2 个元素，而网格有 3 列以上 → **第 3 格会被下一行的元素填进去**，全体错位并多生成行。实测：高度 338→578px、节点右越 33px，而 `--geom-check` 只报"越界"、不会告诉你根因是排布错位。
```css
.ly,.sp{grid-column:1}                      /* ✅ 显式钉住 */
.data,.base,.solve,.out,.chk{grid-column:2}
.fb{grid-column:3;grid-row:3/10}            /* 跨行的侧栏用 grid-row span */
```

**② 扇出/汇聚的横母线端点要按列宽精确算**
母线 `margin` 必须等于**半个列宽**，否则支线戳出小凸头。内容宽 430、4 列 `gap:8` → 列宽 `(430-3×8)/4=101.5` → `margin:0 50.75px`。我写成 53.75 就两端各短 3px。
> ⛔ **验证方法**：给母线支线与目标列打**同一个 `data-mh-col`**，`--geom-check` 的声明式对齐检测会直接报中轴偏差（实测 12 个标记全部同轴才算过）。这是唯一能机械验证"分叉真的对齐"的手段。

**③ `1fr` 在 `width:fit-content` 的父容器里塌成 min-content**
中央轴布局用 `grid-template-columns:1fr 26px 1fr` 会塌到 256px、比例 3.1、占页 85%。**侧列一律给定宽**。

**④ 层标签的竖分隔线要连续**
连接件行的占位格也得带 `border-right`，否则那条线每隔一行断一截，看着像虚线。
```css
.sp{align-self:stretch;border-right:1px solid #000}   /* 占位格也要有线 */
```

**⑤ `min-width` 是压「高/宽」的关键旋钮**
竖向链的行数固定，加宽即直接降比例。实测：不设时 319×368、比例 1.15、占页 65% 不达标；设 `min-width:468px` 后比例 0.79、占页 45%，最小字仍 `468/11=42.5 ≤56` → 两闸皆过。

**⑥ 路线图禁止详细展开到认知过载**
子问题若固定展开成“方法+步骤+产出”三列表，模块常会达到 23~29，而且所有题都会长得一样。`fig_roadmap*` 超过 18 个语义节点、560 个可见字符或单节点超过 64 个字符必须失败；但每个核心节点仍应保留 1～3 条题目特有的关键变换。求解器内部关系另画真实架构图，不能靠删成空提纲过关。

**⑦ 绝对定位的伪元素【计入 scrollWidth】—— 报"文字溢出"却怎么加宽都没用**
判断节点的左右尖角若用 `left:-9px`/`right:-9px` 伸出盒子，`--geom-check` 就报溢出 9px，
**且加宽盒子完全无效**（实测盒宽 250→304，溢出恒定 9px）。这是"加宽治不好的假溢出"的唯一根因。

```css
/* ❌ 尖角伸出盒子：溢出 9px，永远报错 */
.gt{position:relative}
.gt::after{position:absolute;right:-9px;border-left:9px solid #000}
/* ✅ 尖角挂外层 wrapper 的 padding 区内，文字盒本身干净 */
.gtw{position:relative;padding:0 9px}
.gtw::after{position:absolute;right:0;border-left:9px solid #000}
.gt{border:1.3px solid #000}
```

**⑧ 连线端点必须按【列边界】算，不能按"节点边缘"猜**
`right:100%` 的参照系是**轨道列自己**，不是节点。按"从节点边缘往外量"定宽度会错：
实测回边横段落在 386..400 而节点右边界在 353 → **33px 缺口、两头都不接**；
另一条更错成 312..416 → **横段穿进节点内部 41px**（视觉上就是线戳进框里）。

> ✅ **根治办法：让判断/处理节点 `width:100%` 撑满主干列**，右边界恒等于列宽，
> 所有出边端点都变成确定值。横段统一 `left:0; width:<列宽/2>px`，左端贴节点、右端贴竖线。
> ⛔ **`--geom-check` 查不出连线断头**（它只量文字盒），只能出图后按像素核对，或用
> `data-mh-col` 给"支线与目标节点"打同一标记，让声明式对齐检测去验中轴。
> ⛔ 像素核对时注意：**汉字长横与框线会被误判成"断头连线"**，别据此乱改。

**⑨ 分支标签只写「是 / 否」，⛔ 不许加说明 —— 长标签会越出本列压住相邻轨道**
✅ **已补闸**：`--geom-check` 现在有「标签越位」项 —— 判据是**绝对定位的文字是否跑出自己的定位父**
（越出 >12px 即报）。⛔ 为什么不能直接查"文字压连线"：连线是 `::before/::after` 画的伪元素，
**伪元素拿不到 `getBoundingClientRect`**，DOM 层面无法参与重叠检测，只能用这个替代判据。
> 阈值取 12px（≈一个汉字宽）：小幅偏移（如 `left:-6px` 让标签贴近竖线）是正常美观微调，
> 实测不压到任何东西；只有整个标签跑进隔壁列才是真问题。
两次实测（现在都会被闸拦下，分别报越出 34px / 71px）：
- 「否，换种子重解」8 字横跨 88px，直接盖住相邻回边轨道的竖线；
- 「否，继续积分」6 字占 66px，右端落在 x=212.5 而相邻列从 213 起 —— **只差 0.5px 没压穿**。

> ✅ 说明性文字一律并入**上游处理节点的副标题**或图注。ISO 只要求标注分支走向，「否」已足够。
> ⛔ 自查办法：出图后量**列间空隙**的墨行段 —— 正常应只剩横箭头那几行（如 y=179..184），
> 若出现第二段墨（10 行左右的汉字笔画），就是标签越界了。

**⑩ `html,body{width:fit-content}` + 无 grid/flex 撑开 → 出图全白**
实测：只有 `.fig{width:fit-content;padding}` 而内部无 grid/flex 时，capturePage 得到空白图
（报 `image too small (likely blank)`）。模板都有 grid 兜着所以没暴露，改布局时要留意。

**⑪ 最小字号留余量**
详细展开后最小字普遍落在 7.2~7.8pt（下限 7.0）。**层标签别用 10.5px** —— 实测正好卡在 6.9pt 差 0.1pt 不过。统一 11px 起。
### B 配色配方（⛔ 用户选择锁定，不随种子或题型改变）

Step 1 已算出主色相 `H0`（0–359 的整数）。**按下表用 HSL 推导整套色板**，每张图开头写成 `:root` CSS 变量。同一篇论文所有图共用同一 `H0`，所以色板自动统一。

⛔⛔ **核心原则（去"全彩 AI 感"）**：**节点主体不填彩色**，`H0` 推导的彩色**只用于三处**——①全图唯一焦点 ②语义连线/判断分支（是/否、回流）③描边档(TONE 1)的类型边框。**绝不给每个节点填不同颜色**（那正是"太像 AI/PPT 模板"的根源）。一张图里彩色占比目测 ≤15%。

⛔⛔ **以 0.2 节《零灰墨色》为准**：节点主体走**纯黑白**（`#fff` 底 + `#000` 线/字），层次靠 `INK` 轴的线宽/线型/底纹/反色，**不靠灰阶明度差**。下面 B.1 表里的 `--n-bg`/`--n-bg2` 灰阶值**仅在用户显式要求灰阶风格时才用**，默认一律不用。

| 角色 | 变量 | 推导规则（H=色相 S=饱和 L=亮度） | 用途 |
|---|---|---|---|
| ~~灰阶·节点底~~ | `--n-bg` | ⛔ **已废**（违反 0.2 禁灰） | 改用 `#fff` + 纯黑底纹 |
| ~~灰阶·分区底~~ | `--n-bg2` | ⛔ **已废**（违反 0.2 禁灰） | 改用 `#fff` + 边框/底纹区分 |
| **线** | `--rule` | `#000`（⛔ 唯一值，不设浅档） | 节点边框/连线；粗细分层不靠明度分层 |
| **灰阶·正文字** | `--text` | `#1a1a1a`（近黑，无色相） | 节点内文字 |
| **弱文字** | `--tx-sub` | `#000` + 小 2px + `letter-spacing:.04em`（⛔ 原 `#6b6b6b` 违反 0.2；⛔ 原 `opacity:.78` 合成 lum=56 同样违反，已作废） | 副标题/注释 |
| 强调色 | `--accent` | `hsl(H0, 42%, 45%)` | ⛔ **只用于唯一焦点的边+字、语义分支标签、回流箭头**——不填普通节点 |
| 焦点浅底 | `--accent-bg` | `hsl(H0, 40%, 95%)` | 唯一焦点节点的柔和浅底（仅此一个节点可有色底） |
| 类型边框 | `--type-a/-b` | `hsl(H0,38%,48%)` / `hsl(H0,28%,62%)` | **仅 TONE 1(描边档)** 用：同一锁定色相的深浅变化，不引入第二色相 |
| 画布底 | —（无变量） | `transparent` | ⛔ 整图不铺底色 |
...
- 主文字对比度 **≥ 7:1**（深字浅底/白底自然达标）。
- **强调色只 1 种色相（H0 派生），语义连线复用它**；不引入第二种彩色色相（黑白灰不算色相）。
- **B.1 H0 由用户选择固定**：A 恒为 28° 暖棕，B 恒为 224° 靛蓝，C 不使用 H0。不得按学科吸附，不得按工作区换色。H0 只染 ≤15% 的强调部分，不染普通节点主体。
