...
科研工作者最大的画图痛点往往不是"不会用 matplotlib"，而是"手上一堆数据，不知道该用什么图把结论讲清楚"。`fig-visualization-advisor` 是 SciPilot Skills 家族的第二个成员，专做这件事——
...
```
普通画图工具           fig-visualization-advisor
─────────────         ─────────────────────
...
| **fig-visualization-advisor** | **v2.1.0 (本仓库)** | **可视化顾问 + 绘制 + 视觉自检闭环** |
...
The hardest part of scientific plotting is rarely "I don't know matplotlib" — it's "I have data, I don't know which chart conveys my conclusion." `fig-visualization-advisor` is the second member of the SciPilot Skills family, built around this insight: **think first, plot second**.

### Why this isn't just a plotting tool

```
Generic plotter            fig-visualization-advisor
──────────────             ──────────────────────
...
| **fig-visualization-advisor** | **v2.1.0 (this repo)** | **Advisor + renderer + visual self-check loop** |
