"""
fig-visualization-advisor :: export_figure.py
=========================================
...
def export_figure(
    ...
    for fmt in formats:
        if fmt in {"jpg", "jpeg"}:
            print(f"[fig-visualization-advisor] WARNING: skipping {fmt} — "
        ...
        saved.append(path)
        print(f"[fig-visualization-advisor] wrote {path}")
...
def _grayscale_from(fig, basename: str, dpi: int) -> str | None:
    ...
    try:
        ...
        print("[fig-visualization-advisor] Pillow not available; "
    ...
    print(f"[fig-visualization-advisor] wrote {gray_path} (grayscale preview)")
    return gray_path
...
def _cli() -> int:
    p = argparse.ArgumentParser(description="fig-visualization-advisor figure exporter")
