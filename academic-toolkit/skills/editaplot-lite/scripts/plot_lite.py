# -*- coding: utf-8 -*-
"""editaplot-lite：editaplot（Origin 版）方法论的无 Origin 渲染路线（matplotlib 引擎）。

方法论真源：academic-toolkit/skills/editaplot/SKILL.md；配色真源复用其官方 palette-catalog.json。
  1) 逐列角色分类：主证据/可见辅助(误差棒)/保留不画/不确定——不确定列拒画，列为待确认问题；
  2) 科学确认哈希冻结：确认书携带源 sha256 + 列映射，render 强校验，任一变化即拒绝；
  3) 配色取自 editaplot 官方目录，校验系列数 <= max_qualitative_categories 且图型在推荐列表；
...
用法（两段式，对应 editaplot 的 understand→confirm→render 门禁）：
...
PALETTE_CATALOG = SKILLS_DIR / "editaplot" / "assets" / "palettes" / "palette-catalog.json"
...
def classify_columns(df: pd.DataFrame) -> dict:
    """editaplot 纪律：每列必须归入唯一角色；数值列无明确语义时= uncertain（问题，不是新曲线）。"""
    ...
    for c in cols:
        ...
        if base and base in df.columns and pd.api.types.is_numeric_dtype(df[c]):
            ...
            roles[c] = "uncertain"  # editaplot：未知数值列是问题，不是自动曲线
        else:
...
def propose(csv: Path, intent: str) -> dict:
    ...
    proposal = {
        ...
        "note": "uncertain 列在未获用户逐列确认前拒绝渲染（editaplot 纪律）",
    }
...
def validate_confirm(csv: Path, confirm: dict) -> tuple:
    """哈希冻结校验（editaplot 第 3 步）：源内容不变、列集合不变、x/误差棒角色不变；
...
def render(csv: Path, confirm: dict) -> dict:
    ...
    # ── 校验（editaplot 第 8 步的 lite 等价）：产物齐 + 源数据不变 + 白底 ──
