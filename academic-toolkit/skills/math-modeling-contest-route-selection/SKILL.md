...
- **脚本路径**：本技能 scripts/score_topics.py（相对工具箱根 `academic-toolkit/skills/math-modeling-contest-route-selection/scripts/score_topics.py`），Python 3 标准库实现，`python score_topics.py <input.json>` 直接可跑，零第三方依赖。
- **references 加载**：本目录 references/ 10 文件按 SKILL.md Resource Routing 段的触发条件按需加载，路径改为本目录相对路径。
- **触发条件（本仓场景）**：CUMCM/MCM 选题会、AI 选题结果审计、建模路线比较、获奖论文逐问建模蒸馏。

## 输入/输出契约（上游已有，此处摘要）

- 输入：选题评分 JSON（topic/route 级 evidence + flip_condition 按准则分键；fallback 必须含 trigger/action/deadline 三元结构；热门题强制 crowd_escape_mechanism）
- 输出：三层编号结构（A/B/C 选题 12 段 / 单题 9 段 / 深度规划 +4 项）+ score_topics.py 产出的排名表与警告

## 质量铁律（上游 13 条同质化判据 + 7 条强制重写 + 工程 7 问门禁，逐条保留）

任一命中即判泛型并强制重写；工程可行性 7 问任一缺失即拒绝/降级。
