...
- `paper-journal-selection` when the target is `Nature`, `Nature Methods`, `Nature Biotechnology`, or the venue fit among those journals is still unclear
...
1. `paper-bootstrap`
2. `paper-journal-selection` when venue fit or article type is uncertain
