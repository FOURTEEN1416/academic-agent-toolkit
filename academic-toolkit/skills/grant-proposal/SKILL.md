...
- **Feishu notifications are optional.** 若 `~/.acat/feishu.json`存在, send `checkpoint` at each phase transition and `pipeline_done` at final output. If absent, skip silently.
