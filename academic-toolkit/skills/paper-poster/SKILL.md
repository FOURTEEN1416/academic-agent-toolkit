---
name: paper-poster
description: "从已成稿论文生成会议海报（article + tcbposter LaTeX，出 A0/A1 PDF，另出可编辑 PPTX 与 SVG）。触发词：做海报、制作海报、conference poster、make poster、生成poster、poster session。只做海报（A0/A1 单页 + PPTX）；要做逐页幻灯/演讲稿改用 paper-slides。"
...
- **Feishu notifications are optional.** 若 `~/.acat/feishu.json`存在, send notifications. Otherwise skip.
