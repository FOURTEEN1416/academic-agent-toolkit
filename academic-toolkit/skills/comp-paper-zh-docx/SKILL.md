...
- ⛔⛔ **每个独立（块级）公式都必须连续编号，编号一律写在公式内部，用 `\tag{n}`。多行块级公式的闭合 `$$` 必须独占一行、后面不许有任何字符。**

```markdown

模型可表示为：

$$\min_{\mathbf{x}} \sum_{i=1}^{n} c_i x_i \quad \text{s.t.} \quad \sum_i a_{ij} x_i \leq b_j \tag{1}$$

其中 $c_i$ 为第 $i$ 项成本系数。

```

> ⛔ **禁止 `$$ … (1)`（编号粘在闭合定界符前同行尾）与"闭合 `$$` 后仍有字符"** —— 旧版口径
> 「另起一行 `(1)`」+ 示例把 `(1)` 塞进 `$$` 内，两种写法自相矛盾，AI 于是拼出第三种形态。
> Pandoc / Word 导出引擎不认带尾巴的 `$$` 为合法闭合，会从那一行起一路往后吞，
> 把后续正文、公式、`![](figures/xxx.png)` 图片引用全当成数学内容 →
> **公式一个字一个字竖排、图片加载失败**（实测一篇论文 17 处块级公式全中招）。
> `\tag{n}` 是 KaTeX / MathJax / Pandoc 通认的标准写法，右侧照样显示 (1)、(1′) 等编号。

⛔ **禁止 `\begin{equation}`、`\begin{align}`** —— 引擎不处理这些 LaTeX 环境。
⛔ **`\[...\]` 也不要用于独立公式** —— 引擎虽能识别，但会渲染成**行内**公式（不居中、不带编号），达不到独立公式的效果。

✅ **`\begin{cases}` 与 `\begin{aligned}` 可以用**（写在 `$$ ... $$` 里面）—— 引擎会转成 OMML，
Word 里正常显示。0-1 变量定义和"目标函数 + s.t. 多条约束"这两种写法都靠它们，见下面优化模型章节。
<!-- modex-3 同源吸收 P3（2026-09-22）：本段为上游实测缺陷修复口径，精确替换旧「编号另起一行」写法。 -->



#### ⛔⛔ 优化/规划类模型与求解章节写法（docx 公式引擎口径）

**适用范围**：模型里出现 `\min` / `\max`，或题目是"最优/最省/最大化/选址/指派/排班/路径/调度/配比"
这类决策问题时**必须**照此写。**统计建模（评价指标体系 / DEA / PCA / 回归 / 预测）不适用**。

> 依据：最优化问题的三要素就是**决策变量、目标函数、约束条件**。硬要求是三者各自清楚可辨；
> 三者糊成一坨，评委扫一眼找不到"你到底在决定什么"。惯用顺序是决策变量 → 目标函数 → 约束条件 →
> 变量取值范围；**不要求切成三个固定小节**——复杂模型分块或用三线表承载，简单模型一段文字 +
> 一个 `aligned` 公式就够，别为凑结构硬切（数模讲少而精，形式服务于内容）。

**小节标题要带模型信息**（只管子问题章内部小节，章级通行章名照原样保留）：

| ❌ 空标题 | ✅ 带信息的标题 |
|---|---|
| 模型建立 | 基于 0-1 整数规划的服务站选址模型 |
| 模型求解 | 分支切割法求解与最优选址方案 |
| 灵敏度分析（子问题内小节） | 预算上限变动对选址数量的影响 |

**① 决策变量** —— 交代**符号、含义、类型、下标范围、类型理由**五件事。0-1 变量必须用 `cases`
写清两种取值各自的现实含义，不许只写 `$y_j \in \{0,1\}$`：

$$x_{ij}=\begin{cases} 1, & \text{第 } i \text{ 个需求点由第 } j \text{ 个服务站服务} \\ 0, & \text{否则} \end{cases} \tag{1}$$

⛔ 类型理由不能省：取整因人数/车辆不可分，0-1 因选或不选无中间态，连续因可任意分割。

**② 目标函数** —— 独立成式，紧跟一句交代目标来源（引赛题原话，与 `MODELING_REPORT.md`
目标溯源表同口径）：

$$\min\ Z=\sum_{j=1}^{n} f_j y_j+\sum_{i=1}^{m}\sum_{j=1}^{n} c_{ij} d_i x_{ij} \tag{2}$$

多目标要说清怎么合并（加权/分层/Pareto）、权重怎么定、量纲怎么统一。

**③ 约束条件** —— 按容量、守恒、时间窗、逻辑联动等现实机理分组，每组用一个带编号的 `aligned`
公式纵向排开，组后用自然段解释各行业务含义；默认不在成品论文出现 `C1/H1`、"审计项"等内部标签：

$$\begin{aligned}\text{s.t.}\quad & \sum_{j=1}^{n} x_{ij}=1, && i=1,\dots,m \\ & \sum_{i=1}^{m} d_i x_{ij}\le Q_j y_j, && j=1,\dots,n \\ & x_{ij}\le y_j, && \forall i,j\end{aligned} \tag{3}$$

查漏清单：需求覆盖 / 资源预算上限 / 单点容量 / 固定总量 / 至多至少-k / 互斥 / 逻辑联动(big-M) /
时间窗 / 流量守恒 / 度约束与子回路消除。⛔ **线性化技巧必须解释**（big-M、分段线性、绝对值、
`max` 转约束：原非线性形态 + 等价理由；$M$ 取数据上界，不写 99999）。

**④ 变量取值范围 + 模型定性** —— 非负/整数/上下界单独列出；说明问题类型（LP/ILP/MILP/NLP/
多目标/动态规划）、**规模**（按下标范围算出的变量数与约束数）、**为什么用这个解法**（启发式须
说清精确解为何不可行）。⛔ 反例：只丢一串公式与 "使用 Gurobi 求解" 而符号无定义、约束不题面对应、
无类型无规模。**与上游一致性**：目标必须能在 `MODELING_REPORT.md` 溯源表找到对应行，约束条数不
少于建模登记的关键约束数；不新编、不悄漏。

**求解小节**：写的是**为什么这个解可信**，不是算法怎么工作——算法名 + 一句作用 + 引文。
要交代：求解难度与方法选择理由（规模/NP-hard/精确解可行性）、关键实现选择及依据（分解、线性化、
多起点）、**结果的解释边界**（多方法相对差异、与下界/基准对照、误差范围、最紧约束及余量）。
⛔ 不为篇幅写伪代码与复杂度分析（参数进附录表，正文一句"固定随机种子可复现，参数见附录"）；
⛔ 下界不是可行方案，差值不能解读成"优化空间"；⛔ 是否单独设节由题目定，没内容的小节标题比没有更糟；
硬比例：**"解为什么可信"的字数不得少于"算法怎么工作"**。扩写只往建模深度与结果可信度扩。

**创新点必须明确说出来**（位置不限：独立小节、并入优点、或写在结论章）：2-4 条，每条按
"常规做法 → 本文做法 → 带来什么差别"，保持在**建模**层面。✅ 例：用缺车下限重新定义满足率避免
模型退化；把问题一评分注入问题二目标实现两问真正联动。⛔ 判据：换任何题目都成立的话（"用了
机器学习""做了灵敏度分析"）不是本题创新。宁写 2 条实的，不凑 5 条空的。
<!-- modex-3 同源吸收 P3（2026-09-22）：与 comp-paper-zh 的 S8 优化章节口径同源，公式示例已按 docx 引擎（$$ + \tag）改写。 -->
...
- ⛔ DrawIO 图（fig_roadmap / fig_flow_q1/q2/q3 / fig_pipeline）必须嵌入：技术路线图→问题重述末尾；子问题流程图→对应子问题章节开头
...
⛔⛔ **例外：符号说明表必须完整列出，不许用「⋮」省略、也不许挪进附录。** 它是全文符号的唯一索引，
省掉中间行那些符号就再无处可查（实测事故：26 行被截成 8 行）。Word 表格本身能自动跨页。
