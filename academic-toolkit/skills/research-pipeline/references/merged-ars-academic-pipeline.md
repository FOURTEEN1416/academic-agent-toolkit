...
- **Intent**: 在*全新会话*中启动下一阶段（宿主无关）。 Resuming within the same session that emitted the boundary provides no token savings and may drop still-live in-session context.
