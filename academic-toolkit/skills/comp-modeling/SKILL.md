...
**主动推荐·多视角建模（2026-09-11 接线）**：方法选型阶段调 `comp-model-innovation`（arxiv+DuckDuckGo 前沿方法检索 + **历年国一撞方法查重**——多视角建模思路与创新性防线，避免"烂大街方法"直接上手）；实现路线拿不准时用 `model-building`（按子问题选模型→写代码→验证合理性）做快速对照原型。检索与查重结论按留痕纪律写入 MODELING_REPORT.md。
...
- ⛔ **机理/场问题题按需读取**（2026-09-10 同源吸收）：本题涉及**状态相关系数（D(C,T) 变物性）、PDE/场仿真、连续事件/临界值、反问题标定**时，先读 `_utils/mechanism_accuracy_addendum.md`（双副本同 shared-scripts/）对应节——16.2 PDE 场问题（网格收敛/误差分层/收支守恒）、16.1 首达时刻精度、16.3 可辨识性、16.11 本构关系与系数物理含义（D(C,T) 这类系数与它作用后的物理量必须分开）。选影响当前答案的最小有效检查，不全量套用。
...
| "跨问不必交代关系" | 不问衔接是本项目历史上出现过的 fatal（跨问矛盾）。 |
