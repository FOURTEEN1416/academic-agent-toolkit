"""editaplot-lite 契约测试（2026-09-23 收编批次）。
...
SKILL_DIR = REPO / "skills" / "editaplot-lite"
...
def test_skill_files_and_catalog_mapping():
    ...
    entry = next(
        ...
        for e in items
        if isinstance(e, dict) and e.get("capability_id") == "editaplot-lite"
    )
    assert entry["domain"] == "figures_and_document_production"
    assert entry["disposition"] == "routed"
    assert "editaplot-lite" in entry["associated_skills"]
    text = (SKILL_DIR / "SKILL.md").read_text(encoding="utf-8")
    assert 'name: editaplot-lite' in text
