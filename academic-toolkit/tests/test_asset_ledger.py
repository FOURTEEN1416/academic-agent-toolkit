...
def test_classify_path_assigns_conservative_categories() -> None:
    assert classify_path(Path("academic-toolkit/engine/workflow_cli.py"))[0] == "candidate_public_core"
    assert classify_path(Path("academic-toolkit/skills/paper-write/SKILL.md"))[0] == "experimental"
    ...
    assert classify_path(Path("academic-toolkit/.env"))[0] == "excluded_sensitive"


def test_build_ledger_skips_env_content_and_sorts_records(tmp_path: Path) -> None:
    (tmp_path / "academic-toolkit" / "engine").mkdir(parents=True)
    (tmp_path / "academic-toolkit" / ".env").write_text("SECRET=must-not-be-read", encoding="utf-8")
    (tmp_path / "academic-toolkit" / "engine" / "workflow_cli.py").write_text(
        "print('ok')\n", encoding="utf-8"
    )
