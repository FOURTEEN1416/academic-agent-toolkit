"""paper-oral-exemplar 整技能收编契约（2026-09-22 A 批次）。
...
SKILL_DIR = TOOLBOX / "skills" / "paper-oral-exemplar"
...
def test_skill_md_adapter_block_structure() -> None:
    ...
    assert "---\nname: paper-oral-exemplar" in text, "frontmatter name 缺失或不匹配目录名"
    for marker in (
        "本仓适配：输入/输出契约与触发条件",
        "触发条件（本仓场景）",
        "输出契约（两模式二选一，按用户请求）",
...
def test_catalog_entry_mapping() -> None:
    ...
    entries = [e for e in data.get("academic_papers", [])
               if e.get("capability_id") == "paper-oral-exemplar"]
    assert len(entries) == 1, "catalog 缺 paper-oral-exemplar 条目（或重复）"
    entry = entries[0]
    assert entry["domain"] == "academic_papers"
    assert entry["status"] == "experimental"
    assert entry["disposition"] == "routed"
    assert "paper-oral-exemplar" in entry["associated_skills"], "associated_skills 必须含自身"
