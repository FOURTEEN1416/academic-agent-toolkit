...
|------|------------|-------|------------------|--------------|
| 插件结构 | `.claude-plugin/plugin.json` + 组件自动发现 | `.zcode/skills`（NTFS 联结，本地） | `opencode.json` + `.opencode/`（skills paths + plugins） | 根 `opencode.json`（skills.paths 指向 科研工具箱/skills） |
