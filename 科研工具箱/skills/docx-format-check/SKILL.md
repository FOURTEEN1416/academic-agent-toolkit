...
description: "导出 Word 前的 Markdown 格式自检与修复。检查代码块、公式编号、表格、图片、Markdown 残留等会影响 docx 渲染的格式问题，必要时直接修复。"
...
- `- (1)` / `- [1]` 紧跟在公式块后 → 合并到上一条公式末尾，统一成 ` (n)` 形式：`$$ y = ax $$ (1)`。**⛔ 本条旧修法已被取代（modex-3 同源吸收 2026-09-22）：正确目标是把编号移进公式内部改 `\tag{n}`（见 Step 3 开头"上游改判"），块外 ` (n)` 形态本身就要修掉。**
...
**⛔⛔ 上游改判（modex-3 同源吸收，2026-09-22，本节旧口径被本段取代）：唯一允许的公式编号写法是编号写进公式内部用 `\tag{n}`。** 本节下文凡出现"两种写法均合法 / 块外 ` (n)` 可保留 / 修成全角即可"的旧口径，一律按本段执行：

- ✅ 单行：`$$ y = ax \tag{1} $$`；✅ 多行：`$$` 独占一行开块，`\tag{1}` 写在 LaTeX 内，`$$` 独占一行闭合。
- ⛔ **块外编号（`$$ ... $$ (1)`）一律判为错误并修掉**。引擎虽对纯数字编号做了兼容，但一旦编号不是纯数字就会**吞掉后续正文与图片**（事故复盘），所以这种写法不许留在正文里，哪怕当前编号恰好是纯数字。
- 全部要修成 `\tag{n}` 的形态：`$$ y = x $$ (1)`、`$$ y = x $$ （1）`、`$$...$$**（1）**`、`公式（1）：$$...$$`、`$$ y = x $$ \quad (1)`、以及公式块后单独一行的 `(1)` / `- (1)`。
- ⛔ **判据方向：只要闭合 `$$` 那一行后面还有非空白字符，就是问题**——不要只查全角括号。旧版判据写死 `[0-9]+`，`(8′)`、`(A-1)`、`(6) 补充说明` 全部漏检，而漏检的正是最危险的那几种（纯数字编号引擎还能兼容，非纯数字才吞后文）。
- ⛔ **不要只把全角 `（n）` 换成半角 ` (n)`**——那只是从"一定出事"换成"暂时不出事"，编号一旦带撇号（如 `(8′)`）立刻复发。编号一律移进公式内部改成 `\tag{n}`，让开启与闭合的 `$$` 都独占一行、前后不带任何字符。

三条升级 grep（覆盖旧版漏检形态；第①条会把 `$$x=y$$` 这类正常开块行报出来，属正常噪声，逐条看"`$$` 后跟的是编号/文字而不是 LaTeX"才是真问题）：

```bash
# ① 闭合 $$ 带任何尾巴（半角/全角/撇号/带字，全覆盖）——最危险，必修
grep -nE '^[[:space:]]*\$\$[[:space:]]*[^[:space:]]' "$TARGET_FILE" || true
# ② 单行 $$...$$ 后面带尾巴
grep -nE '\$\$.+\$\$[[:space:]]*[^[:space:]]' "$TARGET_FILE" || true
# ③ 公式块后单独一行只写编号（会被当成正文段落）
grep -nE '^[[:space:]]*[-*]?[[:space:]]*[（(][0-9A-Za-z.′'"'"'-]+[)）][[:space:]]*$' "$TARGET_FILE" || true
```

**Check D2 升级（块公式编号扫描的三分类）**：扫描时把块公式分**三类**报出——✅ 已用 `\tag{}`、⛔ 块外编号（要改成 `\tag{}`）、⛔ 完全未编号（要补）。⛔ **块外编号不算"已编号"**——旧版脚本把它算作通过，这类写法一路溜到导出，正是事故的成因。另注意三条实测教训：

1. **文件名走 argv**（`python3 - "$TARGET_FILE" <<'PY'`）：heredoc 是单引号 `<<'PY'`，shell 不展开 `$TARGET_FILE`，写在脚本里会去打开字面名文件（旧版如此，从没跑通过）。
2. **续行块本来就不该单独编号**：一个数学推导常被拆成多个 `$$` 块（第二段以 `=`、`+`、`\leq` 等开头）。不识别续行就会误报成"未编号"，照着报告给每段续行补 `\tag{}`，正文里就出现一串本不存在的公式号——比漏编号更难看（实测真实论文误报 4 处）。⛔ 光看开头字符也不够：`-b \pm \sqrt{b^2-4ac}`（求根公式）、`-\nabla f(x)` 以 `-` 开头却是新公式，须结合上一块结尾判断。
3. **过滤代码块/公式时换成等量空行以保住行号**（`re.sub(r'```[\s\S]*?```', lambda m: "\n"*m.group(0).count("\n"), ...)`）：整段删会让报告行号全部前移（实测偏 3 行、块越多偏得越远），按错行号去找会改错地方。
