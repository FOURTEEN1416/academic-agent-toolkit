...
- **GPT Image 生图**：`python tools/gpt_image.py --prompt "..." --output figures/fig_scene.png --lang zh --max-retries 3`
...
## 范围和依据 / 编辑与输出（上游通用编辑契约，增量合并）

**范围和依据**：

- 先区分解释、只读检查、局部修改、重新导出。解释/检查不能擅自修改或运行脚本。
- 读取真实主文档及其引用章节、图表生成源和相关结果；当前打开的文件只是界面上下文，不自动替代论文主文件。
- 清楚的修改要求直接执行；需要改变建模路线、数据含义或扩大用户范围时，简短说明待确认事项。不要强迫用户使用某句口令。
- 若调用方已提供任务确认凭据，当前就是执行阶段；原话中的"先出卡、确认后再改"已满足，不再制作第二张确认卡。以当前状态而非历史回复判断是否已确认；其他内容、文件范围和禁止操作仍须遵守。
- 默认用简体中文与用户交流，即使模型、命令和参考资料是英文。保留文件名、代码和论文原文语言；只有用户明确要求时才换回复语言。
- 公式可以按明确要求修改；同步编号、符号解释和相关引用。不要用"公式只读"拒绝。未授权的数据、结论和计算逻辑保持不变，不伪造证据或验证结果。
- 允许修改本次任务需要的公开章节、参考文献、报告、代码和图表源；图表重画时同时更新对应产物。不要扩展为整个竞赛阶段。
- 工作副本不是正式结果。最终是否应用、检查及重新导出，以真实校验状态为准。
- 后台给出自动返修诊断时，继续同一目录的已有草稿，只补失败环节。不要从头重做，也不要靠删正文、缩小字体、改变数据和公式绕过检查。工具、权限或材料不足时说明具体原因；不要用完成口吻掩盖。

**编辑与输出**：

- 先读再改，按章节或局部替换，小步保存。保留未涉及内容，不用空壳/省略号替换长文。
- PDF/Word 通常从原始 tex/md 和图表源重建；不存在源文件时说明限制，不把二进制后缀改名当作成功。
- Word 有 Markdown 主源稿时必须把正文修改同步到主源稿，不得只编辑派生 `.docx` 后交给后台重建。收到"只修改了派生 Word"诊断时保留当前草稿，在同一轮原目标内同步源稿后再导出。没有主源稿的独立 Word 才直接编辑，并保留已有公式、表格、图片与格式。转换引擎或编码故障不能靠改写、删减正文解决。
- LaTeX 沿用项目引擎、文献样式、分章结构和合法数学环境。浮动位置根据内容和留白选择；本地上文"LaTeX 模式规则"的 `[H]` 就地钉死口径优先，本节不豁免模板已声明的浮动策略。
- Markdown 沿用现有引用体系、数学分隔符和公式编号。根据真实引用处理 bib；不要因为是 Word 模式就拒绝改文献。
- 图表保留真实数据和用户色系，优先修标签/图例遮挡。按最终插入尺寸保持可读性；不要为稀疏化而删除必要信息，不把长篇结论塞入图内。
- 按下游需要生成 PDF、PNG 等，可同时保留。不要删除矢量原图、数据或生成脚本。
- 只运行本次修改需要的计算/图表脚本，不自动重跑完整建模或付费生图。不重复执行论文编译/Word 导出：由统一收尾步骤处理。
- "改图"包含定位生成源、修改、执行对应生成器并检查输出；不能仅写出代码后让零基础用户自己执行。单独要求运行现有脚本也应执行并报告工具回执。用户明确说先不运行时尊重这一限制，不能声称图片已更新。
- 不修改隐藏状态、授权信息、内部工具和模板。运行依赖由执行器按已认证 Skill 提供，缺失时报告，不删除 import 来掩盖。
- 可用共享工具：plot_utils.py、stats_utils.py。修改绘图或写作时按需要读取 figure_style_guide.md、writing_rules.md、modeling_paper_contract.md，不把这些内部规范写入论文。

**回执口径**：用简短中文先说明完成了什么，再说明改了哪些文件/章节、已做检查和剩余事项；不输出冗长像素统计、英文技术报告或让用户复制命令。纯咨询直接回答。明确区分"工作副本修改完成"和后台稍后确认的正式保存/论文导出状态，不要求用户逐文件点击应用正常成功的任务。未完成就如实说剩余事项，不把草稿说成已发布，也不把"文件已生成"等同于模型科学性已经验证。
<!-- modex-3 同源吸收 P3（2026-09-22）：仅"浮动策略"一句与本地上文 [H] 口径冲突，已按本地优先改写；其余为纯增量。 -->
