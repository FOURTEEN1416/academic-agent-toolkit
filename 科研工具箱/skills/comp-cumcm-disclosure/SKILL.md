...
**适用范围（2026-09-22 G2 注记）**：本双件套机制（确定性脚本 + manifest + 正文声明 + 详情 PDF）
**两族竞赛链通用**——华为杯链的 S14 共用 `comp-final-audit`，其 AI 申报反查闸对
`comp_huawei` 同样按本节 token 执行。但**规则文本的出处是 CUMCM 2026 试行规定**：
华为杯当届若另发 AI 工具使用规定，申报措辞与细则以当届规程为准（provider 白名单与
五类用途闸由脚本统一把守，两族一致），⛔ 勿把国赛规定措辞当作华为杯官方原文引用。
