---
name: editaplot-lite
description: "无 Origin 的出版级数据图路线：继承 editaplot 科学绘图纪律（逐列角色确认、方案冻结、官方配色目录、出版图形合同），用 matplotlib+SciencePlots 把 CSV/TXT 实验表格画成 PNG+PDF+SVG 并附校验报告；先提案逐列用途、经用户确认后渲染；源数据不可变，不确定列拒画，产物如实称 publication-informed lite，不冒充 Origin OPJU。"
---

# EditaPlot Lite · 艾迪图轻量版（无 Origin 的 matplotlib 出版级路线）

> **定位**：本技能是 `科研工具箱/skills/editaplot/`（Origin 可编辑科研绘图）的**方法论继承者**——
> 当本机无 Origin/OriginPro 时，用 matplotlib + SciencePlots 作为渲染引擎，完整继承其
> 科学决策纪律与出版图形合同，产出出版级 PNG/PDF/SVG。
> **方法论真源**：`科研工具箱/skills/editaplot/SKILL.md`（本文件只写差异，冲突时以真源为准）；
> 配色真源复用 editaplot 官方目录 `科研工具箱/skills/editaplot/assets/palettes/palette-catalog.json`。
> **与 editaplot 的关系**：不是上游副本（无 UPSTREAM 溯源件），渲染代码为本仓原创；
> editaplot 路线（OPJU 可编辑工程）见该技能——需要 Origin 工程文件时用它，不需要时用本技能。
...
- 本仓标准 venv：`vendor/forks/editaplot/runtime/.editaplot-venv`（由
  ...
  再 `pip install SciencePlots` 即可；vendor/ 不入库，公开 clone 需按 editaplot SKILL.md §八
  自行获取上游快照，或自建 venv——两者等效，技能只依赖包不依赖路径）。
- 字体：Arial（英文/数字）+ Microsoft YaHei 回退（中文标签）；无 Arial 的机器自动回退系统无衬线体。

## 二、核心流程（三步，对应 editaplot 8 步的 lite 压缩）
...
| 门禁 | 规则 | 继承自 |
|------|------|--------|
| 哈希冻结 | 源 sha256 / 列集合 / 结构角色（x轴·误差棒·保留）与确认书不一致 → 拒绝 | editaplot 第 3 步 |
| 不确定拒画 | 确认书仍有 `uncertain` 列 → 拒绝 | editaplot §四 |
| 配色合同 | 图型须在配色 `recommended_charts`；系列数 ≤ `max_qualitative_categories` | editaplot 官方配色目录 |
| 出版合同 | 白底、Arial+雅黑回退、单栏 9cm（>2 系列双栏 19cm）、无图内标题、图例无框 | editaplot §五 |
| 源数据不可变 | 渲染后源 sha256 必须不变 | editaplot §四 |
| 产物校验 | PNG(300dpi)+PDF+SVG 三件齐 + 白底像素检测 + `verify-report.json` | editaplot 第 8 步 lite 等价 |
...
需要 OPJU 时路由回 `skills/editaplot/`。人工视觉 QA 仍是成功必要条件之一（校验报告不替代看图）。
...
- 竞赛地图：`CONTEST_SKILL_MAP.md` §三 情境可用（editaplot-lite 批次，2026-09-23）；
- 目录：`capabilities/catalog.json` figures_and_document_production 域，disposition=routed；
- 路由表：`科研工具箱/AGENTS.md` §三 科研论文表"没装 Origin 也要出版级数据图"行。
...
契约测试：`科研工具箱/tests/test_editaplot_lite_skill.py`（列分类/门禁负面/渲染校验/catalog 映射）。

## 六、边界（继承 editaplot 三段式适配，本技能无 Origin COM 故风险更低）
