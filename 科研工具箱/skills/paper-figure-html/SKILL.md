...
工作流类型决定数量：
- **数模竞赛 / 科研流程**（有 PROBLEM_ANALYSIS.md）：按清单全部生成（roadmap + flow_q1/q2/pipeline 等），**至少 1 张 fig_roadmap 技术路线图**。⛔ 凡画 `fig_roadmap*`/`fig_technical_route*`，动手前必须完整读取 `references/roadmap_quality.md`（modex-3 同源吸收 2026-09-22；路线图=研究路线总览，不是阶段+方法清单模板，见其 §1）。
...
### Step 4.6: 科研规范自检（modex-3 同源吸收，2026-09-22 · ⛔ 每张必跑，FAIL 必修——抓"缩进论文就糊"）
...
3. **副标题字号压太小**：旧版本曾建议 `.sub` 用 `10-11px`，配宽图必糊。现在 `.sub` **不得低于 11px，且不低于正文 2px 以内**（见 D.1 ③）。
...
⛔ **这里推翻了旧版**。旧版写的是「底纹只加在**支撑节点**、主/焦点节点清掉」—— 但支撑节点**也是有文字的**，照做的结果就是 45° 斜线垫在 11px 中文后面、笔画被网格切碎，再加白描边补救 → 一圈光晕。**这是"图丑"的第一位真因**，详见 0.4 节。`--norm-check` 的「可读性」项现在会硬拦。
...
⛔ **这条推翻了旧建议**（旧版推荐 `opacity:.78`）。理由：黑字 `opacity:.78` 在白底上**合成明度 56**——落在禁灰区间、**肉眼就是深灰**，只是字面量写着 `#000` 骗过了源码扫描。`--norm-check` 的「零灰（合成）」项现在会硬拦它。分界线约在 `.82`，更淡即被拦。
...
/* ❌ .n .sub{ color:#000;opacity:.78 } 合成 lum=56 → 违反本节（旧版曾推荐，已作废） */
```
...
⛔⛔ **本节已被 0.2 节《零灰墨色》覆盖的部分（以 0.2 为准）**：本节旧版把"其余全是灰阶"当作默认答案，**那条已废除**。节点主体走**纯黑白**（`#fff` 底 + `#000` 线/字），层次靠 `INK` 轴的线宽/线型/底纹/反色，**不靠灰阶明度差**。下面 B.1 表里的 `--n-bg`/`--n-bg2` 灰阶值**仅在用户显式要求灰阶风格时才用**，默认一律不用。
