...
| 既有经验库 | ✅ | `科研工具箱/data/contest_lessons.{json,md}`（不存在则按其 schema 新建） |
...
```
缺陷：正文页数超限晚期才发现
  → 强制点：科研工具箱/skills/_utils/quick_gates.py        （门禁前移）
缺陷：审核产物可被主 Agent 手写伪造
  → 强制点：科研工具箱/engine/workflow_runner.py            （requires_subagent 强校验）
缺陷：推荐技能用与不用零留痕
  → 强制点：科研工具箱/engine/workflow_runner.py            （C1 强制申报 + 痕迹绑定）
```
...
python 科研工具箱/tools/contest_lessons_check.py --strict   # schema + 强制点在位 + 双向一致
pytest 科研工具箱/tests/test_contest_lessons.py -q           # 回归护航
```
...
| 产物 | 位置 | 判据 |
|------|------|------|
| 经验库（机器真源） | `科研工具箱/data/contest_lessons.json` | schema 合规、ID 唯一、强制点在位 |
| 经验库（人读真源） | `科研工具箱/data/contest_lessons.md` | 与 JSON ID 集合完全一致 |
