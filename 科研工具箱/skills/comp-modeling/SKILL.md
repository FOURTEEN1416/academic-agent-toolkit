...
**主动推荐·多视角建模（2026-09-11 接线）**：方法选型阶段调 `model-innovation`（arxiv+DuckDuckGo 前沿方法检索 + **历年国一撞方法查重**——多视角建模思路与创新性防线，避免"烂大街方法"直接上手）；实现路线拿不准时用 `model-building`（按子问题选模型→写代码→验证合理性）做快速对照原型。检索与查重结论按留痕纪律写入 MODELING_REPORT.md。
