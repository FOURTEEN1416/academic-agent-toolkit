...
pdf_count = len([f for f in os.listdir("figures") if f.endswith(".pdf")])
total_figs = png_count + pdf_count

print(f"当前图表总数: {total_figs} (PNG: {png_count}, PDF: {pdf_count})")

# 根据参考论文分析（62篇，2962图表）设定阈值
# 简洃型: <30图 (4篇)
# 标准型: 30-50图 (22篇，主流)
# 丰富型: 50-70图 (28篇)
# 2025年篇均图表数达70+
...
if total_figs < 30:
    print("✓ 简洃型：图表数 < 30（符合简洃型标准，4篇样本特征）")
    print("  建议：重点呈现核心结果，避免图表堆砌")
elif 30 <= total_figs <= 50:
    print("✓ 标准型：图表数 30-50（符合主流标准，22篇样本特征）")
    print("  建议：平衡图表与文字，每张图确保有实质性解读")
elif 50 <= total_figs <= 70:
    print("✓ 丰富型：图表数 50-70（符合丰富型标准，28篇样本特征）")
    print("  建议：确保每张图都有深入解读，避免流于形式")
else:
    print(f"⚠ 超过 70 图：当前 {total_figs} 张（2025年样本达 70+，需核对是否符合本题类型）")
    print("  建议：精简至核心图表，或标注为特定题型丰富型")

# 与 FIGURE_MANIFEST 对账
import re
plan_file = ""
for f in ["PROBLEM_ANALYSIS.md", "PAPER_PLAN.md", "MODELING_REPORT.md"]:
    if os.path.exists(f):
        with open(f, 'r', encoding='utf-8') as fp:
            if "<!-- BEGIN FIGURE_MANIFEST -->" in fp.read():
                plan_file = f
                break
...
if plan_file:
    ...
    manifest_figs = re.findall(r'fig_[a-zA-Z0-9_]+', manifest_content)
    drawio_prefixes = ['fig_arch', 'fig_flow', 'fig_roadmap', 'fig_pipeline', 'fig_framework']
    data_manifest = [f for f in manifest_figs if not any(f.startswith(p) for p in drawio_prefixes)]
    manifest_count = len(set(data_manifest))
    print(f"Manifest 数据图计数: {manifest_count}")
    diff = total_figs - manifest_count
    if diff > 0:
        print(f"⚠ 差异: 实际 {total_figs} 张 vs Manifest {manifest_count} 张，差异 {diff} 张")
    elif diff == 0:
        print("✓ 实际数量与 Manifest 一致")
    else:
