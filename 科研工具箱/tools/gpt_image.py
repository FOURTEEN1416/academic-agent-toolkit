...
def _call_gpt_image_api(
    ...
    """调用图像生成 API（兼容 OpenAI / Agnes / SiliconFlow），返回图片 bytes。"""
    ...
    # 模型名：优先显式传入，其次环境变量，默认 gpt-image-2
    if not model:
        model = os.environ.get("GPT_IMAGE_MODEL", "gpt-image-2")

    # 宽高比 → 标准 size（Agnes 支持 size 参数）
    payload_dict = {
        "model": model,
        "prompt": prompt,
        "n": 1,
    ...
    # Agnes 平台：用 extra_body.response_format 返回 base64（官方正确写法）
    if "agnes" in api_base or "agnes" in model:
        payload_dict["extra_body"] = {"response_format": "b64_json"}
