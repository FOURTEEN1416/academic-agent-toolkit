...
ASSETS = [
    {"name": "引用核验器", "path": "科研工具箱/tools/citation_checker.py", "note": "引用条目机检"},
...
def test_stepaction_carries_assets(tmp_path):
    ...
    assert "科研工具箱/tools/citation_checker.py" in instructions
...
def test_used_with_path_trace_passes(tmp_path):
    ...
    result = _complete(
        ...
        commands=[
            ...
            {"command": r"python 科研工具箱\tools\citation_checker.py paper/references.bib",
             "returncode": 0, "cwd": "."},
        ],
...
def test_used_skipped_overlap_rejected(tmp_path):
    ...
    result = _complete(
        runner, wf,
        commands=[{"command": "python 科研工具箱/tools/citation_checker.py", "returncode": 0, "cwd": "."}],
        assets_decl={"used": ["引用核验器"], "skipped": [
