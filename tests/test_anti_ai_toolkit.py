...
# humanize-chinese 机械改写的真实输出（实证捕获的病态模式）
MECHANICAL_REWRITE = """随着人工智能技术的快速进展，大规模语言模型在自然语言处理领域取得了可观进展，近年来，基于Transformer架构的大语言模型在文本生成、机器翻译等任务中展现出强大的能力。要提醒的是，这些模型不仅能够生成流畅自然的文本，还能在一定程度上理解上下文语义，为人工智能的应用开辟了新的可能性。总之，本文的探究为AI生成文本检测给出了新的思路和方法，具有重要的理论价值和实际应用意义，为相关领域的进一步探究奠定了坚实的基础。"""

GOOD_REWRITE = """近年来，大规模语言模型在文本生成、机器翻译等任务中展现出强大的能力。本文针对现有检测方法在语义特征利用上的不足，建立了基于多维特征的分析框架，将语言表达、结构框架与逻辑关系纳入统一评分。实验在多个基准数据集上进行，结果表明本文方法在检出率上优于对比方法。后续将引入更多语料验证其跨领域泛化能力。"""


def test_gate_rejects_mechanical_rewrite():
    """机械改写（语义漂移+杜撰话术）必须被门禁拦截"""
    gate = RewriteQualityGate(AI_SAMPLE, MECHANICAL_REWRITE)
    r = gate.evaluate()
    assert not r.passed, "机械改写应被门禁拦截"
    errors = [i for i in r.issues if i.severity == "error"]
    assert len(errors) >= 2, f"应至少检出2个error级问题: {[i.detail for i in errors]}"
    details = " ".join(i.detail for i in errors)
    assert "语义漂移" in details, "应检出语义漂移"


def test_gate_passes_good_rewrite():
    """有审美的改写（保留语义、无口语化）应通过门禁"""
    gate = RewriteQualityGate(AI_SAMPLE, GOOD_REWRITE)
    r = gate.evaluate()
    assert r.passed, f"好的改写应通过门禁: {r.summary}"
    assert r.score >= 60


def test_de_ai_writer_has_aesthetic_guide():
    """de_ai_writer 必须输出审美守则与受保护术语"""
    engine = DeAiWriter(AI_SAMPLE)
    r = engine.analyze()
    assert "改写审美守则" in r.aesthetic_guide
    assert "语义漂移" in r.aesthetic_guide
    assert r.protected_terms_hit, "应识别出受保护学术术语"
