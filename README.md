...
| ✅ **测试基线** | 仓库根 **639 passed / 0 failed**（本机，2026-09-20 宿主无关改造轮实测；= 工具箱 **620** + 根级门禁 **19**）。唯一真源=`pytest.ini` 注释；覆盖宿主无关协议（boot/probe/forge）、可选适配器、状态机/门禁/审计。历史基线 628/603/460 见 `pytest.ini` 注释（保留作历史；truth-index 为内部文档不入库） |
