...
[![Tests](https://img.shields.io/badge/tests-768_passing-22c55e?style=flat-square&logo=pytest)](科研工具箱/tests)
...
```bash
# 口径一（仓库根，回归门禁口径）：工具箱 743 + 根级门禁 25 = 768 passed / 0 failed
python -m pytest -q
# 口径二（工具箱内，技能验收基线口径）：743 passed / 0 failed
...
| ✅ **测试基线** | 仓库根 **768 passed / 0 failed**（本机，2026-09-22 G2 申报/打包泛化轮实测；= 工具箱 **743** + 根级门禁 **25**；另 3 skipped：私有资料区缺位语义 skip 2 + docx_template_fill pyc 缺陷钉住 1）。唯一真源=`pytest.ini` 注释；覆盖宿主无关协议（boot/probe/forge）、可选适配器、状态机/门禁/审计。历史基线 754/717/651/639/628/603/460 见 `pytest.ini` 注释（保留作历史；truth-index 为内部文档不入库） |
