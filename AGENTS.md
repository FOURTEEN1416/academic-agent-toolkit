...
- `.zcode/config.json` 提供 docsearch MCP（与 OpenCode 同一 server，workspace 级自动连接）与
  **hooks 块（L1 审计，已启用）**：2026-09-11 用户针对"门禁博弈（Goodhart）"威胁裁定重启用，
  ...
  复验记录：契约测试 23/23、引导器 stdin 模拟落账 +1 条、双口径 270/314 全过（同日盲审回炉后基线升至 337/381，见 LOG 续8）。
...
| 运行位置 | 收集范围 | 基线 | 用途 |
|----------|---------|------|------|
| 仓库根 `pytest -q` | `科研工具箱/tests` + 根 `tests/`（pytest.ini 限定） | **381 passed**（2026-09-11 hooks 重启用 0 skip；同日盲审回炉 +67） | 仓库级回归 |
| `科研工具箱/` 内 `pytest -q` | 工具箱自有 tests | **337 passed**（同上） | 技能验收基线（硬规则 3 口径） |
